...
// NewSingleIdleManager returns an singleIdleManager object as the IdleManager interface type to track the watermark idle status.
func NewSingleIdleManager(length int) IdleManager {
	return &singleIdleManager{
...
func (im *singleIdleManager) NeedToSendCtrlMsg(toBufferPartitionName string) bool {
	im.lock.RLock()
...
func (im *singleIdleManager) Get(toBufferPartitionName string) isb.Offset {
	im.lock.RLock()
	defer im.lock.RUnlock()

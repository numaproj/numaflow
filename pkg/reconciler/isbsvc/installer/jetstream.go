...
func (r *jetStreamInstaller) Install(ctx context.Context) (*dfv1.BufferServiceConfig, error) {
	...
	if err := r.createSecrets(ctx); err != nil {
		...
		r.isbSvc.Status.MarkDeployFailed("JetStreamAuthSecretsFailed", err.Error(), r.isbSvc.Generation)
	...
	if err := r.createConfigMap(ctx); err != nil {
		...
		r.isbSvc.Status.MarkDeployFailed("JetStreamConfigMapFailed", err.Error(), r.isbSvc.Generation)
	...
	if err := r.createService(ctx); err != nil {
		...
		r.isbSvc.Status.MarkDeployFailed("JetStreamServiceFailed", err.Error(), r.isbSvc.Generation)
	...
	if err := r.createStatefulSet(ctx); err != nil {
		...
		r.isbSvc.Status.MarkDeployFailed("JetStreamStatefulSetFailed", err.Error(), r.isbSvc.Generation)

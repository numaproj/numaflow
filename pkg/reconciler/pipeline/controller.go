...
func (r *pipelineReconciler) createOrUpdateDaemonDeployment(ctx context.Context, pl *dfv1.Pipeline, isbSvcConfig dfv1.BufferServiceConfig) error {
	...
	// The daemon retains each vertex's initial maximum for pod discovery, while later scale changes do not recreate it.
...
func buildVertices(pl *dfv1.Pipeline, log *zap.SugaredLogger) map[string]dfv1.Vertex {
	...
	for _, v := range pl.Spec.Vertices {
		...
		if isLifecycleChange(pl) {
			...
			partitions := pl.NumOfPartitions(v.Name)
			replicas = int32(partitions)
			// Disable autoscaling for ordered processing vertices by setting min = max = partitions
			vCopy.Scale.Disabled = true
			vCopy.Scale.Min = &replicas
			vCopy.Scale.Max = &replicas
		} else {
			x := vCopy.Scale

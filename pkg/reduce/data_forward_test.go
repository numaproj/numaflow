...
//}
//
//func (f CounterReduceTest) AsyncApplyReduce(ctx context.Context, partitionID *partition.ID, messageStream <-chan *isb.ReadMessage) (<-chan []*isb.WriteMessage, <-chan error) {
//	var (
//		errCh      = make(chan error)
//		responseCh = make(chan []*isb.WriteMessage, 1)
//	)
//	count := 0
...
//}
//
//func (s SumReduceTest) AsyncApplyReduce(ctx context.Context, partitionID *partition.ID, messageStream <-chan *isb.ReadMessage) (<-chan []*isb.WriteMessage, <-chan error) {
//	var (
//		errCh      = make(chan error)
//		responseCh = make(chan []*isb.WriteMessage, 1)
//	)
...
//}
//
//func (m MaxReduceTest) AsyncApplyReduce(ctx context.Context, partitionID *partition.ID, messageStream <-chan *isb.ReadMessage) (<-chan []*isb.WriteMessage, <-chan error) {
//
//	var (
//		errCh      = make(chan error)
//		responseCh = make(chan []*isb.WriteMessage, 1)
...
//	allowedLatency := 1000
//	reduceDataForward, err = NewDataForward(ctx, keyedVertex, fromBuffer, toBuffer, pbqManager, CounterReduceTest{}, f, publisherMap,
//		window, idleManager, op, WithReadBatchSize(int64(batchSize)),
...
//		window, idleManager, op, WithReadBatchSize(1))
//	assert.NoError(t, err)
//
//	// start the forwarder
//	go reduceDataForward.Start()

...
func TestClient_IsReady(t *testing.T) {
	...
	mockClient.EXPECT().IsReady(gomock.Any(), gomock.Any()).Return(&globalreducepb.ReadyResponse{Ready: false}, fmt.Errorf("mock connection refused"))
...
func TestClient_GlobalReduceFn(t *testing.T) {
	var ctx = context.Background()
	...
	messageCh := make(chan *globalreducepb.GlobalReduceRequest)
	close(messageCh)
	responseCh, _ := testClient.GlobalReduceFn(ctx, messageCh)
	for response := range responseCh {
		assert.Equal(t, &globalreducepb.GlobalReduceResponse{
			Result: &globalreducepb.GlobalReduceResponse_Result{
				Keys:      []string{"reduced_result_key1"},
				Value:     []byte(`forward_message`),
				EventTime: timestamppb.New(time.Unix(120, 0).Add(-1 * time.Millisecond)),

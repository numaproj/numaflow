...
func NewUserDefinedSink(vertex *dfv1.Vertex, fromBuffer isb.BufferReader, fetchWatermark fetch.Fetcher, publishWatermark map[string]publish.Publisher, opts ...Option) (*UserDefinedSink, error) {
	...
	isdf, err := forward.NewInterStepDataForward(vertex, fromBuffer, map[string][]isb.BufferWriter{vertex.Spec.Name: {s}}, s.getSinkGoWhereDecider(), applier.Terminal, fetchWatermark, publishWatermark, toVertexPartitionMap, forwardOpts...)
	if err != nil {
		return nil, err
	}
	s.isdf = isdf

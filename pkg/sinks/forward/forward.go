...
func (df *DataForward) forwardAChunk(ctx context.Context) {
	...
	if len(readMessages) == 0 {
		...
		idlehandler.PublishIdleWatermark(ctx, df.fromBufferPartition.GetName(), df.toBuffer, df.wmPublisher, df.idleManager, df.opts.logger, df.vertexName, df.pipelineName, dfv1.VertexTypeSink, df.vertexReplica, wmb.Watermark(time.UnixMilli(processorWMB.Watermark)))
		return
	}

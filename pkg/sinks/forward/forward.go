...
func (df *DataForward) forwardAChunk(ctx context.Context) {
	...
	if len(readMessages) == 0 {
		...
		idlehandler.PublishIdleWatermark(ctx, df.toBuffer, df.wmPublisher, df.idleManager, df.opts.logger, dfv1.VertexTypeSink, wmb.Watermark(time.UnixMilli(processorWMB.Watermark)), df.vertexReplica)
		return
	}

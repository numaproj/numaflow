...
func TestRestfulDaemonClient_GetVertexErrors(t *testing.T) {
	t.Run("error case", func(t *testing.T) {
		...
		client, _ := NewRESTfulDaemonServiceClient(server.URL)
		_, err := client.GetVertexErrors(context.Background(), "simple-pipeline", "out", "0")
		assert.Error(t, err)
	})
	...
	t.Run("okay", func(t *testing.T) {
		server := httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			w.WriteHeader(http.StatusOK)
			_, _ = w.Write([]byte(`{"errors":[{"container":"container1","timestamp":"2023-10-01T12:00:00Z","code":"Internal","message":"message1","details":"details1"}]}`))
		}))
		defer server.Close()

		client, _ := NewRESTfulDaemonServiceClient(server.URL)
		errors, err := client.GetVertexErrors(context.Background(), "simple-pipeline", "out", "0")
		assert.NoError(t, err)
		assert.Len(t, errors, 1)
		assert.Equal(t, "container1", errors[0].Container)
		assert.Equal(t, "Internal", errors[0].Code)
	})
}

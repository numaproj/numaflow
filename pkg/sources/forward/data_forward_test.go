...
func validateMetrics(batchSize int64) (err error) {
	metadata := `
		...
		`
	expected := `
		forwarder_read_total{partition_name="source-partition-0",pipeline="testPipeline",replica="0",vertex="testVertex",vertex_type="Source"} ` + fmt.Sprintf("%f", float64(batchSize)) + `
	`

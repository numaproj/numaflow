...
func TestStatus_IsReadyWithError(t *testing.T) {
	testcases := []struct {
		name       string
	...
	}{
		{
			...
			conditions: []metav1.Condition{
				{Type: string(ConditionReady), Status: metav1.ConditionTrue},
			},
			wantErr: false,
		},
		{
			...
			conditions: []metav1.Condition{
				{Type: string(ConditionReady), Status: metav1.ConditionFalse, Reason: "Error", Message: "Example error"},
			},
			wantErr:  true,
			errorMsg: "condition Ready for monovertex is not true",
		},
		{
			name:       "no conditions set",
			conditions: nil,
			wantErr:    true,
			errorMsg:   "monovertex status conditions not populated",
		},
	}

	for _, tc := range testcases {
		t.Run(tc.name, func(t *testing.T) {
			// Create a status and set conditions
			status := Status{Conditions: tc.conditions}

			// Call the method under test
			err := status.IsReadyWithError()

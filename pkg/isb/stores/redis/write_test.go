...
func TestRedisQWrite_Write(t *testing.T) {
	...
	defer cancel()
	rqw, _ := NewPartitionWrite(ctx, client, "rediswrite", "test", 0, redisclient.WithLagDuration(2*time.Millisecond), redisclient.WithInfoRefreshInterval(2*time.Millisecond)).(*RedisWriter)
...
func TestRedisQWrite_WithPipeline(t *testing.T) {
	...
	stream := "withPipeline"
	count := int64(100)
	rqw, _ := NewPartitionWrite(ctx, client, stream, "test", 0, redisclient.WithoutPipelining()).(*RedisWriter)
	streamName := rqw.GetStreamName()
	defer func() { _ = client.DeleteKeys(ctx, streamName) }()
...
func TestRedisQWrite_WithoutPipeline(t *testing.T) {
	...
	count := int64(100)
	rqw, _ := NewPartitionWrite(ctx, client, stream, "test", 0).(*RedisWriter)
...
func TestRedisQWrite_WithInfoRefreshInterval(t *testing.T) {
	...
	rqw, _ := NewPartitionWrite(ctx, client, stream, group, 0, redisclient.WithInfoRefreshInterval(2*time.Millisecond), redisclient.WithLagDuration(2*time.Millisecond), redisclient.WithMaxLength(10)).(*RedisWriter)
...
func TestRedisQWrite_WithInfoRefreshInterval_WithBufferFullWritingStrategyIsDiscardLatest(t *testing.T) {
	...
	count := int64(10)
	group := "withInfoRefreshInterval-group"
	rqw, _ := NewPartitionWrite(ctx, client, stream, group, 0, redisclient.WithInfoRefreshInterval(2*time.Millisecond), redisclient.WithLagDuration(2*time.Millisecond), redisclient.WithMaxLength(10), redisclient.WithBufferFullWritingStrategy(dfv1.DiscardLatest)).(*RedisWriter)
	err := client.CreateStreamGroup(ctx, rqw.GetStreamName(), group, redisclient.ReadFromEarliest)
	if err != nil {
		t.Fatalf("error creating consumer group: %s", err)
	}
...
func buildTestWriteMessages(rqw *RedisWriter, count int64, startTime time.Time) ([]isb.Message, []string) {
...
func Test_updateIsFullFlag(t *testing.T) {
	...
	rqw, _ := NewPartitionWrite(ctx, client, stream, group, 0, redisclient.WithInfoRefreshInterval(2*time.Millisecond), redisclient.WithMaxLength(10)).(*RedisWriter)
	err := client.CreateStreamGroup(ctx, rqw.GetStreamName(), group, redisclient.ReadFromEarliest)
	defer func() { _ = client.DeleteStreamGroup(ctx, rqw.GetStreamName(), group) }()
...
func Test_GetRefreshFullError(t *testing.T) {
	...
	ctx := context.Background()
	rqw, _ := NewPartitionWrite(ctx, client, "", group, 0).(*RedisWriter)
	expected := uint32(3)
...
func TestNewInterStepDataForwardRedis(t *testing.T) {
	...
	fromStep, _ := NewPartitionReader(ctx, client, fromStream, fromGroup, consumer, 0, redisclient.WithInfoRefreshInterval(2*time.Millisecond)).(*RedisReader)
	_ = client.CreateStreamGroup(ctx, fromStep.GetStreamName(), fromGroup, redisclient.ReadFromEarliest)
	fromStepWrite, _ := NewPartitionWrite(ctx, client, fromStream, fromGroup, 0, redisclient.WithInfoRefreshInterval(2*time.Second), redisclient.WithLagDuration(1*time.Millisecond)).(*RedisWriter)
	...
	to1Read, _ := NewPartitionReader(ctx, client, toStream, toGroup, consumer, 0).(*RedisReader)
	_ = client.CreateStreamGroup(ctx, to1Read.GetStreamName(), toGroup, redisclient.ReadFromEarliest)
	to1, _ := NewPartitionWrite(ctx, client, toStream, toGroup, 0, redisclient.WithLagDuration(1*time.Millisecond), redisclient.WithInfoRefreshInterval(2*time.Second), redisclient.WithMaxLength(17)).(*RedisWriter)
...
func TestReadTimeout(t *testing.T) {
	...
	to1, _ := NewPartitionWrite(ctx, client, toStream, toGroup, 0, redisclient.WithLagDuration(1*time.Millisecond), redisclient.WithInfoRefreshInterval(2*time.Second)).(*RedisWriter)
...
func TestXTrimOnIsFull(t *testing.T) {
	...
	rqw, _ := NewPartitionWrite(ctx, client, buffer, group, 0, redisclient.WithLagDuration(1*time.Millisecond), redisclient.WithInfoRefreshInterval(2*time.Millisecond), redisclient.WithMaxLength(10)).(*RedisWriter)
	err := client.CreateStreamGroup(ctx, rqw.GetStreamName(), group, redisclient.ReadFromEarliest)
	assert.NoError(t, err)
	...
	rqr, _ := NewPartitionReader(ctx, client, buffer, group, "consumer", 0).(*RedisReader)
...
func TestSetWriteInfo(t *testing.T) {
	...
	rqw, _ := NewPartitionWrite(ctx, client, buffer, group, 0, redisclient.WithLagDuration(1*time.Millisecond), redisclient.WithInfoRefreshInterval(2*time.Millisecond), redisclient.WithRefreshBufferWriteInfo(false)).(*RedisWriter)
	err := client.CreateStreamGroup(ctx, rqw.GetStreamName(), group, redisclient.ReadFromEarliest)
	assert.NoError(t, err)
...
func forwardDataAndVerify(ctx context.Context, t *testing.T, fromStepWrite *RedisWriter, to1Read *RedisReader, to1 *RedisWriter, fromStep *RedisReader, f *forward.InterStepDataForward, writeMessages []isb.Message, count int64) {
	stopped := f.Start()
	// write some data

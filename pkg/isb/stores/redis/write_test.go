...
func TestRedisQWrite_Write(t *testing.T) {
	...
	defer cancel()
	rqw, _ := NewBufferWrite(ctx, client, "rediswrite", "test", 0, redisclient.WithLagDuration(2*time.Millisecond), redisclient.WithInfoRefreshInterval(2*time.Millisecond)).(*BufferWrite)
...
func TestRedisQWrite_WithPipeline(t *testing.T) {
	...
	stream := "withPipeline"
	count := int64(100)
	rqw, _ := NewBufferWrite(ctx, client, stream, "test", 0, redisclient.WithoutPipelining()).(*BufferWrite)
	streamName := rqw.GetStreamName()
	defer func() { _ = client.DeleteKeys(ctx, streamName) }()
...
func TestRedisQWrite_WithoutPipeline(t *testing.T) {
	...
	count := int64(100)
	rqw, _ := NewBufferWrite(ctx, client, stream, "test", 0).(*BufferWrite)
...
func TestRedisQWrite_WithInfoRefreshInterval(t *testing.T) {
	...
	rqw, _ := NewBufferWrite(ctx, client, stream, group, 0, redisclient.WithInfoRefreshInterval(2*time.Millisecond), redisclient.WithLagDuration(2*time.Millisecond), redisclient.WithMaxLength(10)).(*BufferWrite)
...
func TestRedisQWrite_WithInfoRefreshInterval_WithBufferFullWritingStrategyIsDiscardLatest(t *testing.T) {
	...
	count := int64(10)
	group := "withInfoRefreshInterval-group"
	rqw, _ := NewBufferWrite(ctx, client, stream, group, 0, redisclient.WithInfoRefreshInterval(2*time.Millisecond), redisclient.WithLagDuration(2*time.Millisecond), redisclient.WithMaxLength(10), redisclient.WithBufferFullWritingStrategy(dfv1.DiscardLatest)).(*BufferWrite)
	err := client.CreateStreamGroup(ctx, rqw.GetStreamName(), group, redisclient.ReadFromEarliest)
	if err != nil {
		t.Fatalf("error creating consumer group: %s", err)
	}
...
func Test_updateIsFullFlag(t *testing.T) {
	...
	rqw, _ := NewBufferWrite(ctx, client, stream, group, 0, redisclient.WithInfoRefreshInterval(2*time.Millisecond), redisclient.WithMaxLength(10)).(*BufferWrite)
	err := client.CreateStreamGroup(ctx, rqw.GetStreamName(), group, redisclient.ReadFromEarliest)
	defer func() { _ = client.DeleteStreamGroup(ctx, rqw.GetStreamName(), group) }()
...
func Test_GetRefreshFullError(t *testing.T) {
	...
	ctx := context.Background()
	rqw, _ := NewBufferWrite(ctx, client, "", group, 0).(*BufferWrite)
	expected := uint32(3)
...
func TestNewInterStepDataForwardRedis(t *testing.T) {
	...
	fromStepWrite, _ := NewBufferWrite(ctx, client, fromStream, fromGroup, 0, redisclient.WithInfoRefreshInterval(2*time.Second), redisclient.WithLagDuration(1*time.Millisecond)).(*BufferWrite)
	...
	to1, _ := NewBufferWrite(ctx, client, toStream, toGroup, 0, redisclient.WithLagDuration(1*time.Millisecond), redisclient.WithInfoRefreshInterval(2*time.Second), redisclient.WithMaxLength(17)).(*BufferWrite)
...
func TestReadTimeout(t *testing.T) {
	...
	to1, _ := NewBufferWrite(ctx, client, toStream, toGroup, 0, redisclient.WithLagDuration(1*time.Millisecond), redisclient.WithInfoRefreshInterval(2*time.Second)).(*BufferWrite)
...
func TestXTrimOnIsFull(t *testing.T) {
	...
	rqw, _ := NewBufferWrite(ctx, client, buffer, group, 0, redisclient.WithLagDuration(1*time.Millisecond), redisclient.WithInfoRefreshInterval(2*time.Millisecond), redisclient.WithMaxLength(10)).(*BufferWrite)
	err := client.CreateStreamGroup(ctx, rqw.GetStreamName(), group, redisclient.ReadFromEarliest)
	assert.NoError(t, err)
...
func TestSetWriteInfo(t *testing.T) {
	...
	rqw, _ := NewBufferWrite(ctx, client, buffer, group, 0, redisclient.WithLagDuration(1*time.Millisecond), redisclient.WithInfoRefreshInterval(2*time.Millisecond), redisclient.WithRefreshBufferWriteInfo(false)).(*BufferWrite)
	err := client.CreateStreamGroup(ctx, rqw.GetStreamName(), group, redisclient.ReadFromEarliest)
	assert.NoError(t, err)

...
func buildJetStreamBufferWriters(ctx context.Context, vertexInstance *dfv1.VertexInstance) (map[string][]isb.BufferWriter, error) {
	// create writers for toVertex's buffer partitions
	writers := make(map[string][]isb.BufferWriter)
	for _, e := range vertexInstance.Vertex.Spec.ToEdges {
...
func buildJetStreamBufferReaders(ctx context.Context, vertexInstance *dfv1.VertexInstance) ([]isb.BufferReader, error) {
	...
	if vertexInstance.Vertex.GetVertexType() == dfv1.VertexTypeReduceUDF {
		...
		if len(fromBufferPartition) == 0 {
			return nil, fmt.Errorf("can not find from buffer")
		}

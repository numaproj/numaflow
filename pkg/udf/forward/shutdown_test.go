...
func TestInterStepDataForward(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name+"_stop", func(t *testing.T) {
			...
			f, err := NewInterStepDataForward(vertexInstance, fromStep, toSteps, myShutdownTest{}, myShutdownTest{}, myShutdownTest{}, fetchWatermark, publishWatermark, wmb.NewSingleIdleManager(len(toSteps)), WithReadBatchSize(batchSize), WithUDFStreaming(tt.streamEnabled))

...
func (u *MapUDFProcessor) Start(ctx context.Context) error {
	...
	if enableMapUdfStream {
		metricsOpts = metrics.NewMetricsOptions(ctx, u.VertexInstance.Vertex, mapHandler, readers)
	} else {
		metricsOpts = metrics.NewMetricsOptions(ctx, u.VertexInstance.Vertex, mapStreamHandler, readers)

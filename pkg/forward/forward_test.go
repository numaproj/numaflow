...
func TestSourceInterStepDataForwardMultiplePartition(t *testing.T) {
	...
	assert.Equal(t, []interface{}{vertex.Spec.Name + "-" + writeMessages[0].Header.ID + "-0-0", vertex.Spec.Name + "-" + writeMessages[2].Header.ID + "-0-0"},
		[]interface{}{vertex.Spec.Name + "-" + readMessages[0].Header.ID, vertex.Spec.Name + "-" + readMessages[1].Header.ID})
	for _, m := range readMessages {
		// verify new event time gets assigned to messages.
		assert.Equal(t, testSourceNewEventTime, m.EventTime)
		// verify messages are marked as late because of event time update.
	...
	assert.Equal(t, []interface{}{vertex.Spec.Name + "-" + writeMessages[1].Header.ID + "-0-0", vertex.Spec.Name + "-" + writeMessages[3].Header.ID + "-0-0"},
		[]interface{}{vertex.Spec.Name + "-" + readMessages[0].Header.ID, vertex.Spec.Name + "-" + readMessages[1].Header.ID})
	for _, m := range readMessages {
		// verify new event time gets assigned to messages.
		assert.Equal(t, testSourceNewEventTime, m.EventTime)

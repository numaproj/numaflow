...
func TestNewInterStepDataForward(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name+"_basic", func(t *testing.T) {
			...
			f, err := NewInterStepDataForward(vertexInstance, fromStep, toSteps, &mySourceForwardTestRoundRobin{}, myForwardTest{}, fetchWatermark, publishWatermark, idleManager, WithReadBatchSize(batchSize), WithUDFStreaming(tt.streamEnabled))
		...
		t.Run(tt.name+"_toAll", func(t *testing.T) {
			...
			idleManager, _ := wmb.NewIdleManager(1, len(toSteps))
			f, err := NewInterStepDataForward(vertexInstance, fromStep, toSteps, &myForwardToAllTest{}, &myForwardToAllTest{}, fetchWatermark, publishWatermark, idleManager, WithReadBatchSize(batchSize), WithUDFStreaming(tt.streamEnabled))
		...
		t.Run(tt.name+"_dropAll", func(t *testing.T) {
			...
			idleManager, _ := wmb.NewIdleManager(1, len(toSteps))
			f, err := NewInterStepDataForward(vertexInstance, fromStep, toSteps, myForwardDropTest{}, myForwardDropTest{}, fetchWatermark, publishWatermark, idleManager, WithReadBatchSize(batchSize), WithUDFStreaming(tt.streamEnabled))
		...
		t.Run(tt.name+"_UDFError", func(t *testing.T) {
			...
			fetchWatermark, publishWatermark := generic.BuildNoOpWatermarkProgressorsFromBufferMap(toSteps)
			idleManager, _ := wmb.NewIdleManager(1, len(toSteps))
			f, err := NewInterStepDataForward(vertexInstance, fromStep, toSteps, myForwardApplyUDFErrTest{}, myForwardApplyUDFErrTest{}, fetchWatermark, publishWatermark, idleManager, WithReadBatchSize(batchSize), WithUDFStreaming(tt.streamEnabled))
		...
		t.Run(tt.name+"_whereToError", func(t *testing.T) {
			...
			fetchWatermark, publishWatermark := generic.BuildNoOpWatermarkProgressorsFromBufferMap(toSteps)
			idleManager, _ := wmb.NewIdleManager(1, len(toSteps))
			f, err := NewInterStepDataForward(vertexInstance, fromStep, toSteps, myForwardApplyWhereToErrTest{}, myForwardApplyWhereToErrTest{}, fetchWatermark, publishWatermark, idleManager, WithReadBatchSize(batchSize), WithUDFStreaming(tt.streamEnabled))
		...
		t.Run(tt.name+"_withInternalError", func(t *testing.T) {
			...
			fetchWatermark, publishWatermark := generic.BuildNoOpWatermarkProgressorsFromBufferMap(toSteps)
			idleManager, _ := wmb.NewIdleManager(1, len(toSteps))
			f, err := NewInterStepDataForward(vertexInstance, fromStep, toSteps, myForwardInternalErrTest{}, myForwardInternalErrTest{}, fetchWatermark, publishWatermark, idleManager, WithReadBatchSize(batchSize), WithUDFStreaming(tt.streamEnabled))
...
func TestNewInterStepDataForwardIdleWatermark(t *testing.T) {
	...
	idleManager, _ := wmb.NewIdleManager(1, len(toSteps))
	f, err := NewInterStepDataForward(vertexInstance, fromStep, toSteps, myForwardTest{}, myForwardTest{}, fetchWatermark, publishWatermark, idleManager, WithReadBatchSize(2))
	assert.NoError(t, err)
	assert.False(t, to1.IsFull())
	assert.True(t, to1.IsEmpty())
...
func TestInterStepDataForwardSinglePartition(t *testing.T) {
	...
	// create a forwarder
	idleManager, _ := wmb.NewIdleManager(1, len(toSteps))
	f, err := NewInterStepDataForward(vertexInstance, fromStep, toSteps, mySourceForwardTest{}, mySourceForwardTest{}, fetchWatermark, publishWatermark, idleManager, WithReadBatchSize(5))
	assert.NoError(t, err)
	assert.False(t, to1.IsFull())
	assert.True(t, to1.IsEmpty())
...
func TestInterStepDataForwardMultiplePartition(t *testing.T) {
	...
	idleManager, _ := wmb.NewIdleManager(1, len(toSteps))
	f, err := NewInterStepDataForward(vertexInstance, fromStep, toSteps, &mySourceForwardTestRoundRobin{}, mySourceForwardTest{}, fetchWatermark, publishWatermark, idleManager, WithReadBatchSize(5))
	assert.NoError(t, err)
	assert.False(t, to11.IsFull())
	assert.False(t, to12.IsFull())
	assert.True(t, to11.IsEmpty())
...
func TestWriteToBuffer(t *testing.T) {
	...
	for _, value := range tests {
		t.Run(value.name, func(t *testing.T) {
			...
			fetchWatermark, publishWatermark := generic.BuildNoOpWatermarkProgressorsFromBufferMap(toSteps)
			idleManager, _ := wmb.NewIdleManager(1, len(toSteps))
			f, err := NewInterStepDataForward(vertexInstance, fromStep, toSteps, myForwardTest{}, myForwardTest{}, fetchWatermark, publishWatermark, idleManager, WithReadBatchSize(value.batchSize), WithUDFStreaming(value.streamEnabled))
			assert.NoError(t, err)

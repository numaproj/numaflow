...
func TestNewTimestampedCounts(t *testing.T) {
	...
	assert.Equal(t, 1, len(tc.podReadCount))
	assert.Equal(t, "{timestamp: 1620000000, podReadCount: map[pod1:map[partition1:10]]}", tc.String())
}

func TestTimestampedCounts_Update(t *testing.T) {
	tc := NewTimestampedCounts(TestTime)
	tc.Update(&PodReadCount{"pod1", map[string]float64{"partition1": 10.0}})
	assert.Equal(t, 10.0, tc.podReadCount["pod1"]["partition1"])
	tc.Update(&PodReadCount{"pod1", map[string]float64{"partition1": 20.0}})
	assert.Equal(t, 20.0, tc.podReadCount["pod1"]["partition1"])
	tc.Update(&PodReadCount{"pod2", map[string]float64{"partition1": 30.0}})
	assert.Equal(t, 30.0, tc.podReadCount["pod2"]["partition1"])
	assert.Equal(t, 2, len(tc.podReadCount))
	tc.Update(nil)
	assert.Equal(t, 2, len(tc.podReadCount))
	assert.Equal(t, 20, int(tc.podReadCount["pod1"]["partition1"]))
	assert.Equal(t, 30, int(tc.podReadCount["pod2"]["partition1"]))

	tc.Update(&PodReadCount{"pod1", map[string]float64{"partition1": 10.0}})
	assert.Equal(t, 10, int(tc.podReadCount["pod1"]["partition1"]))
	tc.Update(&PodReadCount{"pod2", map[string]float64{"partition1": 20.0}})
	assert.Equal(t, 20, int(tc.podReadCount["pod2"]["partition1"]))

	tc2 := NewTimestampedCounts(TestTime + 1)
	tc2.Update(&PodReadCount{"pod1", map[string]float64{"partition1": 40.0}})
	assert.Equal(t, 40.0, tc2.podReadCount["pod1"]["partition1"])
	tc2.Update(&PodReadCount{"pod2", map[string]float64{"partition1": 10.0}})
	assert.Equal(t, 10.0, tc2.podReadCount["pod2"]["partition1"])
}

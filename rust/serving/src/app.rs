...
pub(crate) async fn router_with_auth<T, C>(app: AppState<T, C>) -> crate::Result<Router>
where
    T: Clone + Send + Sync + DatumStore + 'static,
    C: Clone + Send + Sync + CallbackStore + 'static,
{
...
mod tests {
    ...
    async fn test_setup_app() -> Result<()> {
        ...
        let datum_store = InMemoryDatumStore::new(None);

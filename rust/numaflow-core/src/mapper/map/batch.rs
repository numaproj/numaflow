...
impl UserDefinedBatchMap {
    ...
    fn process_response(sender_map: &Arc<Mutex<BatchSenderMapState>>, resp: MapResponse) {
        ...
        if let Some(sender) = sender_entry {
            // Log the error from sending message on oneshot channel instead of panicking
            // which will kill the receiver task.
            // (We should cancel the token if we want to fail the batch on
            // failure to process a single message in the batch)
            if let Err(e) = sender.send(Ok(resp.results)) {
                error!(
                    ?e,
                    "Error sending result from server to batch task on oneshot channel"
                );
            }
        } else {
            warn!(

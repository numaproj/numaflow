...
impl MapStreamTask {
    ...
    ///
    /// For MonoVertex with distributed tracing: wraps the receive loop with a
    /// `numaflow.monovertex.map` span so its duration covers the full stream UDF interaction.
    ...
    async fn execute_inner(mut self) {
        ...
        // Distributed tracing (MonoVertex): inject current `map` span context into
        // sys_metadata["tracing_udf"] so the UDF creates `udf.map.process` as its child.
        // sys_metadata["tracing"] remains unchanged (holds platform.process).
        if is_mono_vertex()

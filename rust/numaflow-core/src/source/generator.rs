...
impl Generator {
    pub(crate) fn new(content: Bytes, rpu: usize, batch: usize, unit: Duration) -> Self {
        let stream_generator = StreamGenerator::new(content, rpu, batch, unit);
        Self { stream_generator }
    }
}

impl source::SourceAcker for Generator {
    async fn ack(&mut self, _: Vec<Offset>) -> crate::error::Result<()> {
        Ok(())
    }
}

impl reader::LagReader for Generator {
    async fn pending(&mut self) -> crate::error::Result<Option<usize>> {
        Ok(Option::from(self.stream_generator.size_hint().0))
    }
}

impl source::SourceReader for Generator {
...
mod tests {
    ...
    async fn test_generator_read() {
        ...
        let mut generator = Generator::new(content.clone(), rpu, batch, unit);

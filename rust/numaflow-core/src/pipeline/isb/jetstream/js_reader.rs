...
mod tests {
    ...
    /// Exploratory test for the duplicate-inflight nack design. Behavior we want to
    /// observe (not a strict pass/fail contract):
    ///
    /// 1. Publish one message, configure AckWait=5s.
    /// 2. First fetch returns delivery-1; we keep its handle in `offset2jsmsg`.
    /// 3. Sleep 6s so AckWait expires and JetStream re-queues the message.
    /// 4. Second fetch returns delivery-2 for the same stream_seq. Note: the second
    ///    fetch overwrites `offset2jsmsg[offset]` with delivery-2's handle — the
    ///    delivery-1 handle is dropped.
    /// 5. Nack delivery-2 with a 1s delay (mirroring the orchestrator's duplicate path).
    /// 6. Attempt to ack the "original" via the reader — observe whether the reader
    ///    can still ack (it won't be able to: the map only has the now-nacked delivery-2
    ///    handle, and nack removes it).
    /// 7. Check pending count to see what JetStream's view is.
    ///
    /// The test prints the observed state via `eprintln!`. It is gated on the
    /// `nats-tests` feature so it only runs when a local NATS server is available.
    #[cfg(feature = "nats-tests")]
    #[tokio::test]
    async fn duplicate_inflight_nack_behavior_against_jetstream() {
        let js_url = "localhost:4222";
        let client = async_nats::connect(js_url).await.unwrap();
        let context = jetstream::new(client);

        let stream = Stream::new("test_dup_nack_behavior", "test", 0);
        let _ = context.delete_stream(stream.name).await;
        context
            .get_or_create_stream(stream::Config {
                name: stream.name.to_string(),
        ...
        // 5s AckWait so we can force redelivery by sleeping 6s.
        let _consumer = context
            .create_consumer_on_stream(
                consumer::Config {
                    name: Some(stream.name.to_string()),
        ...
        let message = Message {
            ...
            event_time: Utc::now(),
            id: MessageID {
                vertex_name: "vtx".to_string().into(),
                offset: "1".into(),
                index: 0,
            },
            ..Default::default()
        };
        let bytes: BytesMut = message.try_into().unwrap();
        context.publish(stream.name, bytes.into()).await.unwrap();

        // First fetch — delivery-1.
        let first = js_reader.fetch(1, Duration::from_secs(2)).await.unwrap();
        assert_eq!(first.len(), 1, "delivery-1 should arrive");
        let offset = first
            .first()
            ...
            .clone();
        eprintln!(
            "dup-nack-test: delivery-1 received, offset={offset:?}, pending={:?}",
            js_reader.pending().await.unwrap()
        );

        // Sleep past AckWait so JetStream re-queues the message.
        tokio::time::sleep(Duration::from_secs(6)).await;

        // Second fetch — delivery-2 (redelivery of same stream_seq). This overwrites
        // `offset2jsmsg[offset]` with delivery-2's handle.
        let second = js_reader.fetch(1, Duration::from_secs(2)).await.unwrap();
        assert_eq!(
            second.len(),
            1,
            "delivery-2 (redelivery) should arrive after AckWait expiry"
        );
        assert_eq!(
            second.first().expect("just asserted len == 1").offset,
            offset,
            "redelivery should carry the same stream_seq → same Offset"
        );
        eprintln!(
            "dup-nack-test: delivery-2 received, pending={:?}",
            js_reader.pending().await.unwrap()
        );

        // Ack delivery-2 (the redelivered/duplicate). This consumes delivery-2's
        // handle from the map. The question: does this ack also close the
        // stream_seq from the broker's perspective, effectively acking the
        // "original" (delivery-1) too?
        js_reader
            .ack(&offset)
            .await
            .expect("ack on delivery-2 should succeed");
        eprintln!(
            "dup-nack-test: acked delivery-2, pending={:?}",
            js_reader.pending().await.unwrap()
        );

        // Wait briefly and re-check pending. If acking delivery-2 closed the
        // stream_seq, pending stays at 0 even though delivery-1's own AckWait
        // was never explicitly resolved.
        tokio::time::sleep(Duration::from_secs(2)).await;
        let pending_after_ack = js_reader.pending().await.unwrap();
        eprintln!("dup-nack-test: pending 2s after ack = {pending_after_ack:?}");
        assert_eq!(
            pending_after_ack,
            Some(0),
            "ack on delivery-2 should close the stream_seq for delivery-1 too"
        );

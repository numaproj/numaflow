...
async fn simple_data_wal_writer() {
    let data_wal = AppendOnlyWal::new("segment", "var/run/numaflow".into(), 20 * 1024 * 1024, 1000, 500).await.unwrap();
    ...
    let (offset_stream, handle) = data_wal.streaming_write(ReceiverStream::new(rx)).await.unwrap();
    // write messages
    for message in messages {
        let _ = tx.send(FileWriterMessage::WriteData {
            id: message.id.to_string(),
            data: message.into(), // impl from for bytes
        }).await;
    }
    
    let gc_wal = AppendOnlyWal::new("gc", "var/run/numaflow".into(), 1 * 1024 * 1024, 1000, 500).await.unwrap();
    ...
    let (tx, rx) = tokio::sync::mpsc::channel(10);
    let (offset_stream, handle) = gc_wal.streaming_write(ReceiverStream::new(rx)).await.unwrap();
    // write gc event
    let _ = tx.send(FileWriterMessage::WriteData {
        id: "gc".to_string(),
        data: gc_event.into(), // impl from for bytes
    }).await;
}

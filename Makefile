...
ifndef SKIP_IMAGE_BUILD
	...
	GRPC_ENFORCE_ALPN_ENABLED=false go test -v -timeout 15m -count 1 --tags test -p 1 ./test/$*
	$(MAKE) cleanup-e2e
...
Test%:
	...
	cat test/manifests/e2e-api-pod.yaml |  sed 's@quay.io/numaproj/@$(IMAGE_NAMESPACE)/@' | sed 's/:latest/:$(VERSION)/' | kubectl -n numaflow-system apply -f -
	GRPC_ENFORCE_ALPN_ENABLED=false go test -v -timeout 15m -count 1 --tags test -p 1 ./test/$(shell grep $(*) -R ./test | head -1 | awk -F\/ '{print $$3}' ) -run='.*/$*'
	$(MAKE) cleanup-e2e

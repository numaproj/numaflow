...
const rawDataToEventListing = (rawData: any[]): K8sEventSummary | undefined => {
  ...
  rawData.forEach((event) => {
    ...
    events.push({
      namespace: event.metadata?.namespace || defaultValue,
      name: event.metadata?.name || defaultValue,
      component: event.source?.component || defaultValue,
      reason: event.reason || defaultValue,
      message: event.message || defaultValue,
      timestamp: event.lastTimestamp || defaultValue,
    });
  });
  return {
    normalCount,
...
export const useNamespaceK8sEventsFetch = ({
  ...
  useEffect(() => {
    ...
    if (fetchData) {
      ...
      const eventList = rawDataToEventListing(fetchData.data);
      setResults({
        data: eventList,
        loading: false,
        error: undefined,

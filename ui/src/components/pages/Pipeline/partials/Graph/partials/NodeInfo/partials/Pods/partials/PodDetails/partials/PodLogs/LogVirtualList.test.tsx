...
describe("LogVirtualList", () => {
  ...
  it("restores scroll position after an ascending prepend", () => {
    const ref = createRef<LogVirtualListHandle>();
    const initialLogs = Array.from({ length: 40 }, (_, i) => `line-${i + 10}`);
    ...
    const list = screen.getByTestId("log-virtual-list");
    ...
    const anchor = ref.current?.captureScrollAnchor();
    expect(anchor).not.toBeNull();

    const prepended = Array.from({ length: 10 }, (_, i) => `line-${i}`);
    rerender(
      <div style={{ height: VIEWPORT_HEIGHT }}>
        <LogVirtualList
          ref={ref}
          logs={[...prepended, ...initialLogs]}
          search=""
          wrapLines={false}
          colorMode="light"
          podName="pod-a"
    ...
    ref.current?.restoreScrollAnchor(anchor!, 10, "asc");
    expect(list.scrollTop).toBe(88 + 10 * 22);
  });
});

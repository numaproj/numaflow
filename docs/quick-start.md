...
2. [Your First MonoVertex](getting-started/monovertex.md) - deploy a single-vertex workload (source to sink, no Inter-Step Buffer Service required).
3. [Your First Pipeline](getting-started/pipeline.md) - connect multiple vertices with edges, from a simple pipeline to an advanced one.
4. [What's Next](getting-started/whats-next.md) - explore more sources, sinks, user-defined functions, and examples.

## Which one should I start with?

A [MonoVertex](core-concepts/monovertex.md) is the simplest way to run Numaflow: a single vertex that reads from a source, optionally transforms the data, and writes to a sink. It needs no Inter-Step Buffer Service and has no edges, so it is the fastest thing to get running. Start here.

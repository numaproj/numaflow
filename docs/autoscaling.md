...
- `replicasPerScale` - Maximum number of replicas is scaled up or down at once, defaults to `2`. For example, if current replica number is 3, calculated desired repilca number is 8, instead of scaling up the vertex to 8, it only does 5.

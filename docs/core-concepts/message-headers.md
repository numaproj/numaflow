...
- **SQS Source Example**  
  When configured with `attributeNames`, SQS system attributes (e.g., `SentTimestamp`, `MessageGroupId`, `MessageDeduplicationId`) are copied into the Numaflow message headers. User-defined message attributes (configured via `messageAttributeNames`) are propagated as user metadata under the `sqs` namespace. Source queue origin is system metadata (group `sqs`, key `queue_name`), not a header; see [Source Queue Origin](../user-guide/sources/sqs.md#source-queue-origin).

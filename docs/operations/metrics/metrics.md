...
| `monovtx_udf_error_total`     | Counter     | Total number of UDF errors              |
...
| `isb_jetstream_max_payload_exceeded_total` | Counter | `pipeline`, `vertex`, `vertex_type`, `replica`, `partition_name` | Total number of JetStream publish attempts exceeding the server-advertised maximum payload                  |

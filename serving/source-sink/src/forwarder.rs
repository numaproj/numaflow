...
use crate::metrics;
use crate::metrics::{
    forward_metrics, FORWARDER_ACK_TOTAL, FORWARDER_READ_BYTES_TOTAL, FORWARDER_READ_TOTAL,
    FORWARDER_WRITE_TOTAL, MONO_VERTEX_NAME, PARTITION_LABEL, REPLICA_LABEL, VERTEX_TYPE_LABEL,
};
        ...
        loop {
            ...
            tokio::select! {
                ...
                result = self.source_client.read_fn(config().batch_size, config().timeout_in_ms) => {
                    ...
                    forward_metrics().forward_read_total.get_or_create(&self.common_labels).inc_by(msg_count);
                    ...
                    // increment the counter read_bytes_total metric from metric rs
                    // Send the common label values as arguments to the get_or_create method
                    // TODO: check best ways to populate labels
                    forward_metrics().forward_write_total.get_or_create(&self.common_labels).inc_by(transformed_msg_count);
                    ...
                    // increment the counter read_messages_count metric from metric rs
                    forward_metrics().forward_ack_total.get_or_create(&self.common_labels).inc_by(msg_count);

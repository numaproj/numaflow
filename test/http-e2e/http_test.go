...
func (s *HTTPSuite) TestHTTPSourcePipeline() {
	...
	w.Expect().RedisSinkContains("out", "no-id", SinkCheckWithContainCount(2))
	...
	w.Expect().RedisSinkContains("out", "with-id", SinkCheckWithContainCount(1))
	...
	w.Expect().RedisSinkContains("out", "with-id", SinkCheckWithContainCount(2))
}

...
func (s *SDKsSuite) TestSourceTransformerJava() {
	w := s.Given().Pipeline("@testdata/transformer/event-time-filter-java.yaml").
		When().
		CreatePipelineAndWait()
	defer w.DeletePipelineAndWait()
	pipelineName := "event-time-filter-java"
...
func (s *SDKsSuite) TestSourceTransformerGo() {
	w := s.Given().Pipeline("@testdata/transformer/event-time-filter-go.yaml").
		When().
		CreatePipelineAndWait()
	defer w.DeletePipelineAndWait()
	pipelineName := "event-time-filter-go"
